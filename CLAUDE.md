...
- **Convergence filter (consensus)** — heatmap/list filter that surfaces only markets where N+ unique top-corpus whales took the same side within a time window (e.g. ≥5 whales bought YES in last 1h). Reduces single-trade noise; turns the heatmap into a "where is smart money agreeing right now" view. Pure aggregation over existing `signals` table — no new ingestion. Hook: tunable threshold (3/5/10 whales) and window (15m/1h/24h) as a UI control alongside metric/range. Optional anti-filter: surface lone elite whale going *against* a forming consensus (contrarian/fade signal).
