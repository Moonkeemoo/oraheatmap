...
- **Sports two-team versus layout** — for sports markets shaped like "Team A vs Team B" (NFL, NBA, esports), render BOTH team icons in tooltip top-markets and L3 row labels (e.g. Fnatic logo + "vs" + Team Vitality logo, Polymarket-style). Backend: detect via subcategory (Sports tags + " vs " in question) and resolve per-team logos — Polymarket's gamma doesn't expose team-level icons, so options are (a) heuristic name → external sports-logo CDN, (b) curated map for top leagues, (c) stick with the single market icon. Single icon ships today; two-team layout is the polish pass once we know which leagues / teams trade most.
